...
<div class="sectionTitle">
	<h2><font color="#CCC"><img src="system/themes/default/images/buttonIcons/options.png" style="margin-bottom:-3px; margin-right:-2px;" width="16" height="16" /> editing</font> <?php echo rawurldecode($post->title()); ?></h2>
...
<form action="" method="post" id="addNewPortfolioItemForm" enctype="multipart/form-data">
	...
	<div class="formItem" id="titleDiv" onMouseOver='visual.changeBackgroundColour(this.id, "#E0E3EF")' onMouseOut='visual.changeBackgroundColour(this.id, "#EEEEEE")'>
		<input type="text" id="title" class="inputDiv" value='<?php echo rawurldecode($post->title()); ?>'/>
	</div>

	<div class="formHeader" style="width: 870px;">Author</div>
	<div class="formItem" id="authorDiv" style='opacity:0.7;'>
		<input type="text" id="author" class="inputDiv" value='<?php echo rawurldecode($post->author()); ?>' readonly/>
	</div>

	<div class="formHeader" style="width: 870px;">Date</div>
	<div class="formItem" id="dateDiv" style='opacity:0.7;'>
		<input type="text" id="date" class="inputDiv" value='<?php echo $post->publishDate(); ?>' readonly/>
	</div>
	...
	<div class="formHeader" style="width: 870px;"><div style="float:left">Preview Image</div><div class="alert" id="fileAlert" style="padding:3px; width:10px; float:right; margin:5px; margin-top:-4px; margin-bottom:0px;"><img src="system/themes/default/images/help.png" /></div></div>
  	<div class="formItem" id="previewImageDiv" onMouseOver='visual.changeBackgroundColour(this.id, "#E0E3EF")' onMouseOut='visual.changeBackgroundColour(this.id, "#EEEEEE")'>
		 	...
		 	<option id="<?php echo $file?>" <?php if(rawurldecode($post->previewImageURL()) == $file) echo 'selected="selected"'?>><?php echo $file?></option>
		 	<?php endforeach; ?>
      		</select>
  	</div>
	...
	<div class="formHeader" style="width: 870px;">Content<div class="alert" id="contentHelp" style="padding:3px; width:10px; float:right; margin:5px; margin-top:-4px; margin-bottom:0px;"><img src="system/themes/default/images/help.png" /></div></div>
  	<div class="formItem" id="descDiv" onMouseOver='visual.changeBackgroundColour(this.id, "#E0E3EF")' onMouseOut='visual.changeBackgroundColour(this.id, "#EEEEEE")' style='border-bottom:1px solid #CCC;'>
		 	<textarea name="description" id="description"><?php echo rawurldecode($post->content()); ?></textarea>
  	</div>

	<input type="hidden" id="categoryID" value="<?php echo $post->tags(); ?>">
	...
	<div id="submitButton" class="submit"
    	 ...
    	 onClick="formData = validateForm('title,author,date,categoryID,previewImage,description'); if(formData) { checkPrivate(); data.updateBlogPost(<?php echo $post->getId(); ?>, formData); $('#sucessMessage').fadeIn(); } ">
    	 Submit
    </div>
    ...
    <p class='checkbox'>Keep Private<input type='checkbox' id='isPrivate' value='true' style='border:none; width:auto; padding:0; height:auto;' <?php if($post->isPrivate() == 'true') echo 'checked="checked"'?>></p>
    ...
    <div id='lastEdited'>
    	Last edited: <b><?php echo $post->lastEdited(); ?></b>
    </div>
